...
namespace jshepler.ngu.mods
{
    ...
    internal class CookingHelper
    {
        ...
        private static bool _autoEat
        {
            get => Options.Cooking.AutoEat.Value;
            set => Options.Cooking.AutoEat.Value = value;
        }
        ...
        private static bool CookingController_Update_prefix(CookingController __instance)
        {
            ...
            if (_autoEat && AutomationThrottle.ShouldRunEveryFrames(ref _lastCheckFrame))
                AutoEatIfReady();

            return false;
        }

        // auto: solve the current meal to 100% efficiency and eat it as soon as it's ready
        private static void AutoEatIfReady()
        {
            var cooking = Plugin.Character.cooking;
            var controller = Plugin.Character.cookingController;

            if (cooking.cookTimer < controller.eatRate())
                return;

            _pairs = null; // re-solve against the current meal's hidden data
            SetOptimalLevels();
        ...
        private static void setColor(Button button)
        {
            ...
            button.image.color = _autoEat ? Plugin.ButtonColor_LightBlue : Color.white;
        }
        ...
        private static bool CookingController_showDishInfo_prefix(CookingController __instance)
        {
            ...
            if ((Menu)character.menuID != Menu.Cooking)
                return false;
        ...
        {
            var ingredients = Plugin.Character.cooking.ingredients;
            for (var i = 0; i < ingredients.Count; i++)
                ingredients[i].curLevel = 0;
        }
        ...
        private static void SetOptimalLevels()
        {
            var ingredients = Plugin.Character.cooking.ingredients;
            for (var i = 0; i < ingredients.Count; i++)
                ingredients[i].curLevel = getOptimalLevel(i);
        }
