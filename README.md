...
| 多资源自动上限分配 | `[AutoAllocation]`(true) | 挂件、Wandoos、NGU 菜单按钮 **Shift+右键**;血魔法菜单 **Shift+右键** 切换仪式魔力 | 每次检查先回收已由自动化管理的分配，再按 `Priority` 重新计算上限并分配;包括挂件能量/升级能量、血魔法魔力、时光机器能量/魔力、高级训练、NGU能量/魔力和 Wishes 的能量/魔力/Res3。Wandoos 特例:参与回收让位，但其 `capAmountEnergy`(=基础时间/速度，每帧恰好升 1 级所需)作为保底先于优先级循环补足，防止归零停摆;当 cap 超过总能量上限(启动期/高 OS 等级)时改为全额注入空闲池;并自动置位原版"开机"状态(`wandoos98On`,该状态无 UI 开关) |
| 黑客自动分配 Res3 | `[AutoAllocation] Hacks`(true) | 黑客按钮 **Shift+右键** 切换(浅蓝) | 黑客解锁后，所有可升级的黑客升级项(未达硬上限、未达手动目标，不含 THE END 黑客)自动补充达到下一等级所需的 Res3;已分配 Res3 常驻不回收，只按检查间隔补足差额，与许愿一致;同时开启许愿自动化且 `Priority` 中 Hacks 排在 Wishes 之前时，黑客合计最多占用 Res3 总容量的一半，剩余留给许愿 |
...
| 烹饪自动就食 | `[Cooking] AutoEat`(true) | 烹饪按钮 **Shift+右键** 切换(浅蓝) | 餐就绪(`cookTimer` 达到 `eatRate`)时自动求解当前餐 100% 效率最佳配比并食用;每次食用前强制按当前餐数据重建配对求解,新餐就绪后再次自动执行;帧轮询节流检查 |
...
| 烹饪辅助(自动求解最佳配比) | 食材名显示 `P{配对号}: 名字 ({食材目标}\|{配对目标})`;**Shift+点击** 食材 `-` 按钮全部归零、**Shift+点击** `+` 按钮自动枚举 0–20 求解并设为 100% 餐效率最优值;按住 Alt 高亮已达最优的配对;菜信息 tooltip 显示烹饪加成数学分解 |
...
| `[AutoAllocation]` | `Priority` | `Yggdrasil,Augment,BloodMagic,TimeMachine,AdvancedTraining,Wandoos,NGU,Hacks,Wishes` | 全局资源分配顺序;缺少或重复项会自动补齐默认顺序 |
| `[AutoAllocation]` | `Augment` | `true` | 挂件能量和升级能量自动分配 |
| `[AutoAllocation]` | `BloodMagic` | `true` | 血魔法仪式魔力自动分配 |
| `[AutoAllocation]` | `Wandoos` | `true` | Wandoos 能量和魔力自动分配;`capAmountEnergy`(=原版 Cap 参考量)保底先补,cap 超过总能量的启动期改为全额注入;自动置位原版开机状态 |
| `[AutoAllocation]` | `NGU` | `true` | NGU 能量和魔力自动分配 |
| `[AutoAllocation]` | `Hacks` | `true` | 黑客自动补充 Res3 至下一等级所需;已分配常驻不回收,Hacks 在 `Priority` 中排在 Wishes 前且许愿自动化开启时,黑客合计最多占用 Res3 总容量的一半 |
...
| `[Cooking]` | `AutoEat` | `true` | 餐就绪时自动求解最佳配比并食用 |
